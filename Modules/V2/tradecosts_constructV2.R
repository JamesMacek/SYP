...
library(tidyverse)
library(R.matlab)
##Estimate_trade_costs. Date Created: October 8th 2021
##                      Date Edited: October 13th 2021

##INPUT: Raw trade flow data, GDP data, etc.
##OUTPUT: Regional aggregated tradeflows + Bilateral trade costs WITHOUT TRADE-ELASTICITY WEIGHT!. Also gives non-aggregated trade costs for international/province
# for the calibration of productivity. 
...
map <- c(6, 2, 8, 5, 7, 5, 8, 8, 5, 3, 1, 6, 6, 6, 7, 4, 6, 1, 1, 7, 7, 3, 4, 7, 6, 8, 2, 7, 8, 4, 9) #Mapping provinces to regions
...
for (hr in Tradeflows) {
  ...
  for (reg1 in colnames(hr)) {
    ...
    for (reg2 in colnames(hr)) {
      i2 = i2 + 1
      hr[i1, i2] <- sqrt(temptr[map[i1], map[i1]]*temptr[map[i2], map[i2]]/(temptr[map[i1], map[i2]]*temptr[map[i2], map[i1]]))
...
setwd("C:/Users/James/Dropbox/SchoolFolder/SYP/data/MyData/constructed_outputV2")
...
#Lastly: calculating disaggregated Head-Riess index for international/province pairs only for use with productivity calibration
dis_tau = data.frame(matrix(ncol = 6, nrow=31))
colnames(dis_tau) <- c("Ag_2002", "Ag_2007", "Ag_2012", "Na_2002", "Na_2007", "Na_2012")

tr_ag_2002 <- as.matrix(tr_ag_2002)
dis_tau$Ag_2002 <- sqrt(diag(tr_ag_2002)*tr_ag_2002["International", "International"]/(tr_ag_2002[, "International"]*tr_ag_2002["International",]))
tr_ag_2007 <- as.matrix(tr_ag_2007)
dis_tau$Ag_2007 <- sqrt(diag(tr_ag_2007)*tr_ag_2007["International", "International"]/(tr_ag_2007[, "International"]*tr_ag_2007["International",]))
tr_ag_2012 <- as.matrix(tr_ag_2012)
dis_tau$Ag_2012 <- sqrt(diag(tr_ag_2012)*tr_ag_2012["International", "International"]/(tr_ag_2012[, "International"]*tr_ag_2012["International",]))

tr_na_2002 <- as.matrix(tr_na_2002)
dis_tau$Na_2002 <- sqrt(diag(tr_na_2002)*tr_na_2002["International", "International"]/(tr_na_2002[, "International"]*tr_na_2002["International",]))
tr_na_2007 <- as.matrix(tr_na_2007)
dis_tau$Na_2007 <- sqrt(diag(tr_na_2007)*tr_na_2007["International", "International"]/(tr_na_2007[, "International"]*tr_na_2007["International",]))
tr_na_2012 <- as.matrix(tr_na_2012)
dis_tau$Na_2012 <- sqrt(diag(tr_na_2012)*tr_na_2012["International", "International"]/(tr_na_2012[, "International"]*tr_na_2012["International",]))

dis_tau["province"] <- colnames(hr)
write_xlsx(dis_tau, "International_tradecosts.xlsx")
