...
for (reg1 in Aggregate_reg) {
  ...
  for(reg2 in Aggregate_reg) {  
    ...
    for (i in reg1) {
      ...
      source_ti <- as.matrix(source_i*(master_data$Urban_spend_2000[i]*master_data[i, "Na_nomY_2000"] + master_data$Rural_spend_2000[i]*master_data[i, "Ag_nomY_2000"]))
      source_tri <- source_ti + source_tri
      gr_gdp <- as.matrix(master_data$Urban_spend_2000[i]*master_data[i, "Na_nomY_2000"] + master_data$Rural_spend_2000[i]*master_data[i, "Ag_nomY_2000"]) + gr_gdp
    }
    
    tr_ag_2002_agr[i1, i2] <- source_tri/gr_gdp
...
for (reg1 in Aggregate_reg) {
  ...
  for(reg2 in Aggregate_reg) {  
    ...
    for (i in reg1) {
      ...
      source_i <- as.matrix(source_i*((1-master_data$Urban_spend_2000[i])*master_data[i, "Na_nomY_2000"] + (1-master_data$Rural_spend_2000[i])*master_data[i, "Ag_nomY_2000"]))
      source_ti <- source_i + source_ti
      gr_gdp <- as.matrix((1-master_data$Urban_spend_2000[i])*master_data[i, "Na_nomY_2000"] + (1-master_data$Rural_spend_2000[i])*master_data[i, "Ag_nomY_2000"]) + gr_gdp
    }
...
for (reg1 in Aggregate_reg) {
  ...
  for(reg2 in Aggregate_reg) {  
    ...
    for (i in reg1) {
      ...
      source_ti <- as.matrix(source_i*(master_data$Urban_spend_2005[i]*master_data[i, "Na_nomY_2005"] + master_data$Rural_spend_2005[i]*master_data[i, "Ag_nomY_2005"]))
      source_tri <- source_ti + source_tri
      gr_gdp <- as.matrix(master_data$Urban_spend_2005[i]*master_data[i, "Na_nomY_2005"] + master_data$Rural_spend_2005[i]*master_data[i, "Ag_nomY_2005"]) + gr_gdp
    }
    
    tr_ag_2007_agr[i1, i2] <- source_tri/gr_gdp
...
for (reg1 in Aggregate_reg) {
  ...
  for(reg2 in Aggregate_reg) {  
    ...
    for (i in reg1) {
      ...
      source_ti <- as.matrix(source_i*((1-master_data$Urban_spend_2005[i])*master_data[i, "Na_nomY_2005"] + (1-master_data$Rural_spend_2005[i])*master_data[i, "Ag_nomY_2005"]))
      source_tri <- source_ti + source_tri
      gr_gdp <- as.matrix((1-master_data$Urban_spend_2005[i])*master_data[i, "Na_nomY_2005"] + (1-master_data$Rural_spend_2005[i])*master_data[i, "Ag_nomY_2005"]) + gr_gdp
    }
    
    tr_na_2007_agr[i1, i2] <- source_tri/gr_gdp
...
for (reg1 in Aggregate_reg) {
  ...
  for(reg2 in Aggregate_reg) {  
    ...
    for (i in reg1) {
      ...
      source_ti <- as.matrix(source_i*(master_data$Urban_spend_2010[i]*master_data[i, "Na_nomY_2010"] + master_data$Rural_spend_2010[i]*master_data[i, "Ag_nomY_2010"]))
      source_tri <- source_ti + source_tri
      gr_gdp <- as.matrix(master_data$Urban_spend_2010[i]*master_data[i, "Na_nomY_2010"] + master_data$Rural_spend_2010[i]*master_data[i, "Ag_nomY_2010"]) + gr_gdp
    }
    
    tr_ag_2012_agr[i1, i2] <- source_tri/gr_gdp
...
for (reg1 in Aggregate_reg) {
  ...
  for(reg2 in Aggregate_reg) {  
    ...
    for (i in reg1) {
      ...
      source_ti <- as.matrix(source_i*((1-master_data$Urban_spend_2010[i])*master_data[i, "Na_nomY_2010"] + (1-master_data$Rural_spend_2010[i])*master_data[i, "Ag_nomY_2010"]))
      source_tri <- source_ti + source_tri
      gr_gdp <- as.matrix((1-master_data$Urban_spend_2010[i])*master_data[i, "Na_nomY_2010"] + (1-master_data$Rural_spend_2010[i])*master_data[i, "Ag_nomY_2010"]) + gr_gdp
    }
    
    tr_na_2012_agr[i1, i2] <- source_tri/gr_gdp
