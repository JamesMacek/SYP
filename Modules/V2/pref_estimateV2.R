...
#NOTE: cost_calibrateV2.m calculates relative cost measures. These get absorbed in intercepts when taking logs-- so the estimates of the parameters epsilon+eta
...
relemp_reg <- lm_robust(log_relemp~log_relprice + log_rY + FE_2005 + FE_2010, data = master, cluster = year,  se_type = "stata", alpha = 0.05)
...
#BOOTSTRAPPING OUTPUT (or Loading bootstrap .Rda)
set.seed(4321)
if (loadBootstrap == 0) {
  estimate_unc_bootstrap <- boot(data = master_sub_na, statistic = estimate_un_boot, R = 5000)
  #saving bootstrap output
  save(estimate_unc_bootstrap, file = "constructed_outputV2/unc_bootstrap_Robject.Rda")  
}  else {
  load("constructed_outputV2/unc_bootstrap_Robject.Rda")
}
...
#Checking correlation with real income. Correlation > 90 percent... wow. 
...
init <- c(2.7, 1.2, 0, 0, 0, 0) #Set initial value near point estimate
...
if (loadBootstrap == 0) {
  ...
  #saving bootstrap output
  save(estimate_nls_bootstrap, file = "constructed_outputV2/nls_bootstrap_Robject.Rda")  
  
  
}  else {
  load("constructed_outputV2/nls_bootstrap_Robject.Rda")
}
...
#Correlation > 75 percent between real income measures implied by the model and elsewhere. 
...
#Exporting with GGplot (correlation figure)
