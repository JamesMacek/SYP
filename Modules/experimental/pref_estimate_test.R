...
#Estimate model where errors are the preference shocks, without using relative employment
estimate_con_experrors <- function(data, ini) { 
  
  obj_fn <- function(x) {
    data["Errors"] <- data$log_agspend - x[1]*data$log_naspend - (x[1] - 1)*(1-x[2])*data$log_EPa - (1-x[2])*data$log_relprice - x[3]*data$FE_2005 - x[4]*data$FE_2010 - x[5]*data$FE_ag - x[6]
    SSE <- colSums(as.matrix(data$Errors)^2)
    return(SSE)
  }
  
  
  estimate_unc <- fmincon(x0 = ini, fn = obj_fn, lb = c(0.001, 0.001, -50, -50, -50, -50))
  return(estimate_unc)
...
init_experrors <- c(2, 1.1, 0, 0, 0, 0)
...
estimate_experrors <- estimate_con_experrors(master, init_experrors)
...
for (r in c(1:50)) {  
  ...
  {
    next #Skips if nlscon fails (due to machine precision error or..?)
  }
